        ...
        try {
	      ...
	      if (!isNumber(user.lastpago)) user.lastpago = 0 
              ...
              if (!user.rtrofi) user.rtrofi = 'Bronce' 
                    ...
                    if (!('label' in user)) user.label = 'Nutzer von Yuuki konno'
            ...
            const isSupporter = isMods || global.mods.map(v => v.replace(/^[0-9]/g, '') + '@s.whatsapp.net'). includes(m.sender)
            ...
            for (let name in global.plugins) {
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    if (plugin.stv && !isstv) { // Stv-inhaber
                        fail('stv', m, this)
                        continue
                    }
                  ...
                  if (plugin.support && !isSupport) { // supporter
                        fail('support', m, this)
                        continue
                  }
                  if (plugin.Hoster && !isHoster) { // Hostar
                        fail('Hoster', m, this)
                        continue
                  }
...
I am *Yuuki konno*, Welcome to ${metadata.subject}.
...
Ich hoffe, wie sehen uns bald Wieder!
`
    ...
    async groupsUpdate(groupsUpdate, fromMe, m) {
        ...
        for (let groupUpdate of groupsUpdate) {
            ...
            //console.log('=============\n\ngroupsUpdate \n\n============\n\n' + await groupUpdate)
            if (!text) continue
            await this.sendButton(id, text, wm, 'Deaktivieren Sie Funktionen', `.off detect`, global.ftroli, { contextInfo: { mentionedJid: this.parseMention(text) }, mentions: await this.parseMention(text) })
        }
    },
